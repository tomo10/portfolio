defmodule Portfolio.Accounts do
  ...
  def user_lifecycle_action("after_register", user, _) do
    Portfolio.Logs.log_async("register", %{user: user})
    Portfolio.Orgs.sync_user_invitations(user)

    Portfolio.Slack.message("""
    :bust_in_silhouette: *A new user joined!*
    *Name*: #{PortfolioWeb.Helpers.user_name(user)}

    #{PortfolioWeb.Router.Helpers.admin_users_url(PortfolioWeb.Endpoint, :edit, user)}
    """)
  end

  def user_lifecycle_action("after_confirm_email", user, _) do
    Portfolio.Logs.log_async("confirm_email", %{user: user})
    Portfolio.Orgs.sync_user_invitations(user)
  end

  def user_lifecycle_action("after_sign_in", user, %{ip: ip}) do
    Portfolio.Logs.log_async("sign_in", %{user: user})
    {:ok, user} = update_last_signed_in_info(user, ip)
    Portfolio.MailBluster.sync_user_async(user)
  end
  ...
  def user_lifecycle_action("after_impersonate_user", user, %{
    ...
    Portfolio.Logs.log_async("impersonate_user", %{user: user, target_user_id: target_user_id})
  ...
  def user_lifecycle_action("after_update_profile", user, _) do
    Portfolio.Logs.log_async("update_profile", %{user: user})
    Portfolio.MailBluster.sync_user_async(user)
  end

  def user_lifecycle_action("after_confirm_new_email", user, _) do
    Portfolio.Logs.log_async("confirm_new_email", %{user: user})
    Portfolio.Orgs.sync_user_invitations(user)
  end

  def user_lifecycle_action("request_new_email", user, %{new_email: new_email}) do
    Portfolio.Logs.log_async("request_new_email", %{user: user, metadata: %{new_email: new_email}})
  end

  def user_lifecycle_action("after_passwordless_pin_sent", user, %{pin: pin}) do
    Portfolio.Logs.log("passwordless_pin_sent", %{user: user})
